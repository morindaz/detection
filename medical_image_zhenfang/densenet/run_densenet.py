...
from densenet.load_data import load_data
...
if __name__ == '__main__':
    ...
    X_train, Y_train, X_valid, Y_valid, Y_img_name = load_data('./train_img/ap_train_1', './train_img/ap_label.xlsx', (img_rows, img_cols), num_classes, train_ratio)
    # Load our model
    model = densenet121_model(img_rows=img_rows, img_cols=img_cols,
                     color_type=channel, num_classes=num_classes, growth_rate=16, nb_dense_block=2)
    ...
    predictions_valid_label = np.argmax(Y_valid, axis=1)
    ...
    for i in range(len(Y_valid_label)):
        if Y_valid_label[i] != predictions_valid_label[i]:
            print('image:{0}\t real:{1}\tpredict:{predictions_valid_label}'.format(Y_img_name[i], Y_valid_label[i], predictions_valid_label[i]))
