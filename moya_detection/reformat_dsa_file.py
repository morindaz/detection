...
class ReformatDsaFile(object):
    def __init__(self, data_path, result_path, resize, sop_ids=[], patients_info={}):
    ...
    def read_file(self, file_path):
        try:
            dsa = dicom.read_file(file_path, force=True)
            ...
            if position == 'AP':
                dir_prefix = moya_diag.predDicom(frameArray = image_array, manufacturer = manufacturer)
            elif position == 'LAT':
                dir_prefix = position
            else:
                dir_prefix = sopUid
    ...
    def save_info(self):
        ...
        info_path = './result/' + str(len(os.listdir('./result'))+1)
        os.makedirs(info_path)
        with open(os.path.join(info_path, 'Sop_ids.json'), 'wb') as f:
            f.write(sop_ids_json)
            f.close()
        with open(os.path.join(info_path, 'Sop_ids_bak.json'), 'wb') as f:
            f.write(sop_ids_json)
            f.close()
        with open(os.path.join(info_path, 'patients_info.json'), 'wb') as f:
            f.write(patient_info_json)
            f.close()
        with open(os.path.join(result_path, 'patients_info_bak.json'), 'wb') as f:
            f.write(patient_info_json)
            f.close()
...
if __name__ == '__main__':
    ...
    info_path = './result/' + str(len(os.listdir('./result')))
    patient_info_path = os.path.join(info_path, 'patients_info.json')
    sop_ids_path = os.path.join(info_path, 'Sop_ids.json')
    patient_info = {}
    sop_ids = []
    ...
    rdf = ReformatDsaFile(root_path, result_path,(750, 750),  sop_ids=sop_ids, patients_info=patient_info)
    # rdf.read_file('../IM000001')
    rdf.reformat_file(root_path)
    rdf.save_info()
